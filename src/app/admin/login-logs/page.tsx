...
export default function AdminLoginLogsPage() {
  ...
  useEffect(() => {
    if (isAdmin) {
      adminService.getLoginLogs().then((data) => { setLogs(data); setDataLoading(false); });
    }
  }, [isAdmin]);
  ...
  const filtered = useMemo(() => logs.filter((log) => {
    ...
    return true;
  }), [logs, outcome, search, dateFrom, dateTo]);
                    ...
                    return (
                      <tr key={log.id} className="border-b border-border/30 hover:bg-primary/5 transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-white whitespace-nowrap">
                          {log.new_value?.email || (log.investor_id ? `${log.investor_id.slice(0, 8)}…` : '—')}
                        </td>
